	...
	filter(input:ListFilter):ListItem[] {
		...
		.filter(({ item }):boolean => {
			...
			if(input.contextsAnd !== undefined) {
				if(input.contextsAnd.filter((context) => {
			...
			if(input.contextsOr !== undefined) {
				if(input.contextsOr.filter((context) => {
					return contexts.includes(context);
				}).length === 0) {
					return false;
				}
			...
			if(input.contextsNot !== undefined) {
				if(input.contextsNot.filter((context) => {
					return contexts.includes(context);
				}).length > 0) {
					return false;
				}
			...
			if(input.projectsAnd !== undefined) {
				if(input.projectsAnd.filter((context) => {
			...
			if(input.projectsOr !== undefined) {
				if(input.projectsOr.filter((context) => {
					return projects.includes(context);
				}).length === 0) {
					return false;
				}
			...
			if(input.projectsNot !== undefined) {
				if(input.projectsNot.filter((context) => {
					return projects.includes(context);
				}).length > 0) {
					return false;
				}
