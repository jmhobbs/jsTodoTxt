...
const sampleCompleted = 'x (Z) 2022-10-17 2022-09-03 We should keep +todoItems in their @place when rendering out due:2022-10-22';
...
test('toAnnotatedString › Returns the correct ranges', t => {
	...
	const item = new Item(itemStr);
	const annotated = item.toAnnotatedString();
	t.deepEqual(annotated.contexts.map(ctx => ctx.string), ['@wall']);
	t.deepEqual(annotated.projects.map(prj => prj.string), ['+painted']);
	t.deepEqual(annotated.extensions.map(ext => ext.string), ['color:blue']);
